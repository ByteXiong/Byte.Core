...
namespace Byte.Core.Common.Extensions
{
    ...
    public static class ExpressionExtensions
    {
        ...
        /// <returns></returns>
        public static Expression<Func<T, T>> And<T>(this Expression<Func<T, T>> first, Expression<Func<T, T>> second)
        {

            NewExpression newBody = Expression.New(typeof(T));
            MemberInitExpression oldExpression = (MemberInitExpression)first.Body;
            ParameterExpression[] oldParamters = first.Parameters.ToArray();

            MemberInitExpression newExpression = (MemberInitExpression)second.Body;
            //ParameterExpression[] newParamters = second.Parameters.ToArray();
            ...
            //newExpression.Bindings.Where(x => !existsProperties.Contains(x.Member.Name)).
            //    ForEach(aProperty =>
            //    {

            //        MemberInfo newMember = aProperty.Member; //newExpression.Bindings.GetMethod(aProperty.Member.Name);
            //        MemberBinding newMemberBinding = Expression.Bind(newMember, Expression.PropertyOrField(oldParamters[0], aProperty.Member.Name));
            //        bindings.Add(aProperty);
            //    });
            bindings.AddRange(newExpression.Bindings);
            var body = Expression.MemberInit(newBody, bindings.ToArray());
            var resExpression = Expression.Lambda<Func<T, T>>(body, oldParamters);
