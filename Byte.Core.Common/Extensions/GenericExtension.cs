...
namespace Byte.Core.Common.Extensions
{
    ...
    public class ObjectFastClone<TIn, TOut>
    {
        ...
        public static Func<TIn, TOut> GetFunc()
        {
            ...
            {
                var tInFullName = typeof(TIn).GetRuntimeProperty(item.Name)?.PropertyType.FullName;
                var tOutFullName = typeof(TOut).GetRuntimeProperty(item.Name)?.PropertyType.FullName;
                if (tInFullName == tOutFullName)
                {
