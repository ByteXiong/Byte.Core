...
namespace Byte.Core.Common.Extensions
{
    public static class EntityFrameworkCoreExtensions
    {
        ...
        public static DataTable GetTableColumns(this IDbContextCore context, params string[] tableNames)
        {
            ...
            else if (db.IsOracle())
            {
                sql = "select a.Table_Name as TableName,"
                    ...
                    //  + "decode(a.IDENTITY_COLUMN, 'YES', 'TRUE', 'NO', 'FALSE') as IsIdentity,"//是否自增
                    + "'FALSE' as IsIdentity,"
                      ...
                      + $"where a.Table_Name in ({tableNames.Select(m => $"'{m}'").Join(",")})";//.ToUpper()
            }
            else
            {
                throw new NotImplementedException("This method does not support current database yet.");
        ...
        public static IList<DbTable> GetCurrentDatabaseTableList(this IDbContextCore context, Func<DbTable, bool> selector = null)
        {
            ...
            tables.ForEach(item =>
            {
                //var sql = $"select* from[{item.TableName}] where 1 != 1";
                ...
                var dt = context.GetDataTable(sql);
        ...
        /// <returns></returns>
        public static IList<DbTable> GetDatabaseTableList(this IDbContextCore context)
        {
        ...
        public static void ClearDataTables(this IDbContextCore context, params string[] tables)
        {
            ...
            context.ExecuteSqlNoQuery(sql.ToString());
        }
